...
void displayScoreAndLives(int score, int lives) {
  char scoreBuffer[16];  // OLED has 16 characters per row
  char livesBuffer[16];
  
  // Score on row 0: "Score: 1234"
  snprintf(scoreBuffer, sizeof(scoreBuffer), "Sc:%-5d", score);
  writeDisplay(scoreBuffer, 0, true);  // Row 0
  
  // Lives on row 1: "Lives: 3"
  snprintf(livesBuffer, sizeof(livesBuffer), "Lv:%d", lives);
  writeDisplay(livesBuffer, 1, true);  // Row 1
}
...
void displayTopScores(int topScores[3]) {
  char buffer[16];
  snprintf(buffer, sizeof(buffer), "Top:%d %d %d", 
          topScores[0], topScores[1], topScores[2]);
  writeDisplay(buffer, 2, true);  // Row 2
}
...
void displaySensitivity(int sensitivity) {
  ...
  writeDisplay(buffer, 3, true); // Display on row 2 and clear the row
}
